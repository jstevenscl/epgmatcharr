...
- **GN Station Matcher** — dedicated page to assign Gracenote station IDs (`tvc_guide_stationid`) directly to channels; scored candidates, staged commit, country filter (US/GB/DE/NL and more), a sticky channel group filter that's remembered across sessions, and a clear button to remove bad mappings
...
- **Emby Sync reset** — clear every managed channel's Emby guide mapping in one click, instead of deleting each one by hand in the Emby UI; respects excluded groups and tuner scoping
...
- **Backfill on commit** — optionally write matched GN station IDs or tvg-ids back to Dispatcharr channels at commit time; force-overwrite checkboxes let you replace an existing value too, with a preview of every change shown before you confirm
...
1. Assign GN station IDs to your channels first, using **GN Matcher**.
...
3. Open **Emby Sync** and click **Preview Coverage** to see what would be mapped, with no changes made to Emby.
...
Want a clean slate instead? The **Delete all guide data from Emby** button on Emby Sync clears every managed channel's mapping in one click — a reset for when mappings have gotten into a bad state, instead of removing each one by hand in the Emby UI.
