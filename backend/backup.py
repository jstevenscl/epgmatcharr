...
component (a config/cache file or a database) can be downloaded, restored
from a previously-downloaded file, or reset to a fresh empty state
independently of the others -- e.g. a corrupt GN station DB can be wiped
without touching saved credentials, or the config can be rolled back
without discarding the EPG cache.
...
is itself recoverable. SQLite components are downloaded via a VACUUM INTO
snapshot rather than copying the raw file, so an in-progress write elsewhere
in the app can never produce a torn/corrupt download.
"""
...
_COMPONENTS = [
    _Component("config", "config.json", "Configuration (Dispatcharr connection, EPG settings, login credentials)", "json"),
    _Component("sessions", "sessions.json", "Active login sessions", "json"),
    _Component("epg_cache", "epg_cache.json", "EPG cache (rebuilds automatically from Dispatcharr if reset)", "json"),
    _Component("gn_station_db", "gn_station_db.sqlite", "Gracenote station DB (re-download from Settings if reset)", "sqlite"),
]
